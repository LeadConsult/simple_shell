...
void parse_ln(char *line, size_t size, int command_counter, char **av)
{
	...
	if (read_len != -1)
	{
		...
		if (i == -1)
			create_child(param_arr, line, command_counter, av);
 ...
 * * @av: Name of --> program running --> shell
 */
void create_child(char **param_arr, char *line, int count, char **av)
{
	pid_t id;
	int status;
	int i;
 ...
 * * @delim: A constant string containing desired delimeter to tokenize line
 * * @token_count_: A holder for --> amount of tokens in a string.
 * * Return: Upon success an array of tokens representing command.
 * * Otherwise returns NULL.
 */
char **token_interface(char *line, const char *delim, int token_count_)
{
	char **param_arr;

	token_count_ = count_token(line, delim);
	if (token_count_ == -1)
	{
		free(line);
		return (NULL);
	}
	param_arr = tokenize(token_count_, line, delim);
	if (param_arr == NULL)
	{
		free(line);
		return (NULL);
 ...
 * * Otherwise returns NULL.
 */
char **tokenize(int token_count_, char *line, const char *delim)
{
	int i;
	char **buffer;
	char *token;
...
/**
 * * count_token - Counts tokens in --> passed string.
 * * @line: String that is separated by an specified delimeter
 * * @delim: --> desired delimeter to separate tokens.
